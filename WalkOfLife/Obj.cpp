	...
	if (obj_File.is_open()){
		while (std::getline(obj_File, line)){
			if (line[0] == 'v' && line[1] == ' '){
				...
				verteciesPos.push_back({ x, y, z * -1.0f });
				vertexPositions.push_back(XMFLOAT3(x, y, z * -1.0f)); //storar alla vertexpositioner i en separat array som sedan kan användas i main

			}

			else if (line[0] == 'v' && line[1] == 't' && line[2] == ' '){
				sscanf_s(&line[0], "vt %f %f", &u, &v);
				verteciesUV.push_back({ u, 1.0f - v });
			}

			else if (line[0] == 'v' && line[1] == 'n' && line[2] == ' '){
				sscanf_s(&line[0], "vn %f %f %f", &nx, &ny, &nz);
				verteciesNor.push_back({ nx, ny, nz * -1.0f });
			}
